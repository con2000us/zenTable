...
| # | 項目 | 說明 |
|---|------|------|
| 1 | zentable_renderer.py 支援 --page, --sort, --asc, --desc，並可接受陣列輸入 | 在 zentable_renderer.py 新增 ROWS_PER_PAGE、normalise_data()（陣列 of 物件 → `{headers, rows}`）、apply_sort_and_page()；CLI 解析 --page、--sort、--asc、--desc；load_json 之後先 normalise 再 sort/page，再送入 CSS/PIL/ASCII 三種渲染。 |
| 2 | 主題 cyberpunk 與文件一致 | 新增 `themes/css/cyberpunk/template.json`（skill 與專案皆有），配色對齊 SKILL.md 所述 cyberpunk 風格。 |
| 3 | 更新 SKILL.md 反映 zeble_render.py、themes/、雙入口 | 在 skill 與專案 doc/SKILL.md 中補上：zentable_renderer.py 為對外主入口（支援 CSS/PIL/ASCII、--theme-name、--transparent、--page、--sort 等）；zeble.py 為 CLI 用；themes/ 來自 themes/css/、themes/pil/、themes/text/；測試頁透過 theme_api + gentable_*.php 呼叫 skill。 |
...
| 附 | gentable_*.php 改為優先調用 skill 的 zentable_renderer.py | gentable_css.php、gentable_pil.php、gentable_ascii.php 皆改為：`file_exists($scriptSkill)` 時用 skill 版，否則 fallback `doc/zeble_render.py`。 |
...
│  │  zentable_renderer.py     │     │  gentable_pil.php│     │  ...                     │  │
...
│                                                                                     │
│  ┌──────────────────────────────────────────────────────────────────────────────┐ │
│  │  scripts/zentable_renderer.py  (主渲染程式)                                             │ │
...
| 渲染腳本 | doc/zeble_render.py（fallback） | skill zentable_renderer.py |
