...
| **render_pil 內建 theme_colors** | zeble_render.py | PIL 的內建主題顏色 dict（default_dark, default_light, cyberpunk, gradient_modern, glass） |
| **render_pil theme_name_map** | zeble_render.py | 前端 theme id 對應內建 key（dark→default_dark, light→default_light 等） |
...
| 項目 | 位置 | 說明 |
|------|------|------|
| **ASCII_STYLES** | zeble_render.py | 框線字元實作（single/double/grid/markdown），由 theme 的 `params.style` 選擇使用哪一種，非主題定義本身 |
